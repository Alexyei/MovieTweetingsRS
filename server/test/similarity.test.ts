...
import {calculateMoviesSimilarity} from "../src/builders/calculate_movies_similarity";
import {calculateUsersSimilarity} from "../src/builders/calculate_users_similarity";
beforeAll(async () => {
...
function zerosM(rows: number, columns: number) {
    return Array(rows).fill(0).map(() => Array(columns).fill(0));
}

function sumRow(list: number[]) {
    return list.reduce((partialSum, a) => partialSum + a, 0);
}

function stdRow(list: number[]) {
...
test('item-similarity otiai builder', async () => {
    const sims = calculateMoviesSimilarity(data,0.2,4)
    console.log(sims)
    expect(sims.length).toBe(2)
    expect(sims[0].similarity).toBeCloseTo(0.786, 2)
    expect(sims[1].similarity).toBeCloseTo(0.958, 2)
})

test('user-similarity otiai builder', async () => {
    const sims = calculateUsersSimilarity(data,0.2,4)
    console.log(sims)
    expect(sims.length).toBe(5)
    expect(sims[0].similarity).toBeCloseTo(0.755, 2)
    expect(sims[1].similarity).toBeCloseTo(0.964, 2)
    expect(sims[2].similarity).toBeCloseTo(0.218, 2)
})
