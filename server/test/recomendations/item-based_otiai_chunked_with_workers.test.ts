...
import {
    calculateMoviesOtiaiSimilarityChunkedWithWorkers, calculateMoviesOtiaiSimilarityChunkedWithWorkersAsyncConveyor
} from "../../src/builders/calculate_movies_otiai_similarity_chunked";
...
export async function createMovieSims(movieSimsData: {
    source: string,
    target: string,
    similarity: number,
    type: "OTIAI"
...
async function getUniqueMovieIds(skip: number, take: number) {
    return prisma.testRating.findMany({
        distinct: ['movieId'],
        orderBy: {movieId: 'asc',},
        select: {movieId: true},
        skip: skip,
...
test('create item similarity', async () => {
    const usersData = await prisma.testRating.groupBy({
        by: 'authorId',
        _avg: {rating: true},
    ...
    await calculateMoviesOtiaiSimilarityChunkedWithWorkersAsyncConveyor(usersData, uniqueMovieIds, getChunkRatings, saveChunkSims, 2000, 11, 0.2, 4)
})
