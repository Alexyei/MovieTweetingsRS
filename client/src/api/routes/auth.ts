...
class AuthApi extends ApiHelper {
    ...
    async SignUp(payload: SignUpPayloadT) {
        ...
        return this._fetchWrapper<{ status: 201, response: UserT }, SignUpPayloadT>( URL, 'POST', payload)
    }

    async SignIn(payload: SignInPayloadT) {
        const URL = this._API_URL + '/auth/login'
        return this._fetchWrapper<{ status: 200, response: UserT }, SignInPayloadT>( URL, 'POST', payload)
    }
    ...
    async UserData(userId?: number){
        ...
        return this._fetchWrapper<{ status: 200, response: UserT} ,null>( URL, 'GET')
